...
const EditarProducto = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      precio: Yup.number().min(0.01, 'El precio debe ser mayor que 0').required('El precio es obligatorio'),
    ...
    onSubmit: async producto => {
      ...
      try {
        const refDoc = doc(firebase.db, "productos", id);
  ...
  useEffect(() => {
    const cargarProducto = async () => {
      try {
        const refDoc = doc(firebase.db, "productos", id);
        const producto = await getDoc(refDoc);
